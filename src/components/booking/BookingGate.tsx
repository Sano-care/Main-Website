...
//   NEXT_PUBLIC_OTP_DEFAULT_CHANNEL    — "whatsapp" (current) or "sms"
//   NEXT_PUBLIC_WHATSAPP_OTP_ENABLED   — "true" to allow WhatsApp Cloud API direct
...
function pickPrimaryChannel(): Channel {
  ...
  // Fall-through priority: whatsapp > sms. The market is WhatsApp-heavy
  // and the template message renders better than SMS.
  if (WHATSAPP_ENABLED) return "whatsapp";
  if (SMS_ENABLED) return "sms";
  // No flag is set in env — fall back to "whatsapp" so the UI still
...
// When the primary is WhatsApp, the only useful cross-modal fallback is
// SMS. When the primary is SMS, the fallback is WhatsApp if enabled.
